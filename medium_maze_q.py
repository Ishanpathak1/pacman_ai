...
class QLearningAgent:
    ...
    def train(self, layout_name='mediumSearch', episodes=1000, render_last=True):
    ...
    def test(self, layout_name='mediumSearch', render=True):
        layout = getLayout(layout_name)
        game_state = GameState()
        game_state.initialize(layout, numGhostAgents=0)
    ...
    def save_model(self, filename="q_table_medium.pkl"):
        with open(filename, "wb") as f:
            pickle.dump(dict(self.q_table), f)
        print(f"📎 Q-table saved to '{filename}'.")

    def load_model(self, filename="q_table_medium.pkl"):
        try:
...
if __name__ == "__main__":
    ...
    try:
        print("🤖 Training Q-Learning Agent on layout 'mediumSearch'...")
        agent.train(episodes=1000, render_last=True)
        agent.save_model("q_table_medium.pkl")
        print("✅ Training completed! Now running test...")
        agent.test(layout_name='mediumSearch', render=True)
